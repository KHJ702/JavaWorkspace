...
public class C_Scanner {
	...
	public void inputTest1() {
		...
		String firstname = sc.next(); // 성[스페이스]이름[엔터]
		String lastname = sc.nextLine(); //엔터값을 기준으로 엔터 앞에있는 모든 글자를 가져옴
	...
	public void inputTest2() {
		...
		/*
		 * 주의사항
		 * 1) sc.nextXXX() : 메서드들 뒤에 sc.nextLine()메서드가 와야 될 경우
		 * 					 sc.nextLine()을 한번 더 호출하여 버퍼를 비워줘야 한다. (중요)
		 * 2) 스캐너를 통해 '문자'값을 입력 받아야 될 경우
		 * 	  sc.nextLine()을 통해 "문자열"을 입력받고, 그후 charAt(인덱스)를 호출한다.
		 */
