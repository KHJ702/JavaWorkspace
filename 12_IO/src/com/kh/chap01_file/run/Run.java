...
public class Run {
	public static void main(String[] args) {
		...
		try {
			...
			// 4. File에서 제공하는 메서드들.
			System.out.println(bbbFolder.isFile()); // Folder이므로 false
		 ...
		 * 반드시 프로그램과 외부매체간의 데이터가 오고갈 수 있는 "통로(Stream)"를 개설해야 한다.
		 * 이 통로를 Stream이라고 부른다. (스트림은 데이터를 전달하는 통로다!)
		 * 
		 * 스트림의 특징
		 ...
		 * 스트림의 구분
		 *  - 데이터의 크기에 따른 분류
		 *		1) 바이트 스트림(InputStream/OutputStream) <- (1바이트) 바이트스트림은 인풋스트림 아웃풋스트림으로 불림
		 *		 - 모든 바이트 스트림은 InputStream/OutputStream 추상클래스를 상속받아 구현. (인풋,아웃풋 스트림 = 추상클래스)
		 *		 - 읽기는(read()), 쓰기는(writer())를 사용.
		 *		 - 1byte짜리 데이터가 오고갈 수 있는 스트림.
		 *
		 *
		 *  	2) 문자 스트림(Reader/Writer) <- (2바이트) 바이트스트림은 리더스트림 라이터스트림으로 불림
		 *		 - 모든 문자 스트림은 Reader/Writer 추상클래스를 상속받아 구현.
		 *		 - 읽기는(read()), 쓰기는(writer())함수를 사용.
