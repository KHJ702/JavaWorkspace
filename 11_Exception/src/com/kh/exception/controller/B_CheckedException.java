...
/*	
   try {
		// readLine메서드 사용시 IOException에 대한 예외처리를 반드시 해줘야 컴파일 에러가 없어딘다.
...
*/	
	...
	// 2. throws : 현재 메서드에서 예외를 처리하지 않고, 현재 메서드를 호출하는 메서드로 예외를 throws하겠다.
	//	   		   예외처리를 위임한다.
	String str = br.readLine();
	System.out.println("문자열의 길이 : "+str.length());
	  ...
	  * IOException : 예외가 프로그램 실행 전 발생 (컴파일 에러) => 예외처리가 필수다.
	  * 			  반드시 예외처리 구문(try~catch, throws)으로 예외를 해결해줘야 한다.
	  * 
	  */
}
