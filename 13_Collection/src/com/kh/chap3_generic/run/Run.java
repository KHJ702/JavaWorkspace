...
import com.kh.chap3_generic.model.vo.Child;
...
public class Run {
	...
	public static void main(String[] args) {
		...
		*/
		...
		// 3. 제네릭의 extends (!!여기서부터 어려워짐)
		//  - 제네릭의 extends는 클래스간의 상속개념이 아닌, 제네릭으로 지정한 자료형을 "제한" 시키는데 사용한다.
		//  - <T extends Parent> : 타입변수에는 Parent클래스 및 Parent클래스를 상속받은 자료형만 제시할 수 있다.
		//  - <T extends Interface> : Interface를 구현한 클래스만 타입변수에 제시 가능.
		
		Generic2<Parent> parent = new Generic2<>();
		Generic2<Child> child = new Generic2<>();
		//Generic2<String> str = new Generic2<>(); <- Parent의 자식이 아니므로 객체 생성 불가.
		// 제네릭 => 타입을 제한시키고 그 타입이 아닌 객체를 생성하면 에러를 발생시킨다.
		...
		/* 4. 와일드카드(?)
		 ...
		 - 와일드카드(?)의 의미는 어떤 자료형이 들어올지 모른다(unknown)의 의미로 사용. ?가 어떤 자료형이든 올수 있다는(any)의 의미가 아니다.
		 - ?에는 super와 extends를 사용하여 와일드카드의 범위를 제한할 수 있다.
		 - ? extends T : 상한제한 => 와일드 카드의 범위를 T또는 T의 자식들로 제한
		 - ? super T   : 하한제한 => 와일드 카드의 범위를 T또는 T의 조상들로 제한
		...
		// unknown2.setT(new Child1()); // ? 범위에 Child2도 포함되어있으므로 "대입 불가" (Child1만 만들어도 불가 <- 컴파일러는 자식클래스가 몇인지 모름 바보)
		
		Parent p = unknown2.getT(); // ?(와일드)가 Parent이든, 그 자식이든 다형성에 의해 값을 추출하여 대입 가능.
		// extends를 통한 타입 제한은 데이터를 가져올 때 사용한다.
		
		// 2) super를 통한 와일드카드 범위 제한 : 하한제한
