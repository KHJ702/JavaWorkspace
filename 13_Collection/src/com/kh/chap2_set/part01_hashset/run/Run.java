...
public class Run {
	...
	public static void main(String[] args) {
		/*
		 * Set
		 *  - 중복을 허용하지 않는 자료구조 (!Set의 중요한 특징! -> 중복값을 허용하지 않는다.)
		 *  - 저장된 순서를 보장하지 않는다.
		...
		ArrayList<String> list = new ArrayList<>(hs1); // 컬렉션의 데이터를 배열에 모두 담은 상태로 컬렉션 생성 (리스트 생성)
		list.addAll(hs1);
		System.out.println(hs1);
		 ...
		 *  2. 저장한 위치에 이미 저장된 값이 있다면 eqauls()를 통해 저장된 값과 새로 추가할 값이 같은지 검사한다.
		...
		set1.add(111); // current 111, next : 1
		set1.add(1); // current 1 next : null
		System.out.println(set1);
	
		// TreeSet -> 정렬
		// set내부에 데이터가 추가될 때 compare, compareTo메서드를 자동 실행.
		Set<Integer> set2 = new TreeSet<>(set1);
		// ArrayList, set을 매개변수로 추가시 안에 담긴 데이터의 기본정렬방식대로 정렬
		// Comparator객체를 추가하면, 이후 set에 데이터를 추가할 때 Comparator에 정의한 정렬조건대로 정렬이 이루어진다.
