...
public class Run {
	 ...
	 * 		- 값을 저장할 때 인덱스를 사용하지 않기 때문에 저장순서가 유지되지 않는다.
	 * 		- key값은 중복을 허용하지 않으나, value값 중복을 허용한다.
	...
	public static void main(String[] args) {
		...
		// (classcast <-형변환 오류. 이 오류는 컬렉션을 사용할 때 자주 발생. 오류를 줄이기 위해 타입을 고정하고 제네릭을 사용 함.)
		// 제네릭<E>
		// 컬렉션에 저장할 객체의 타입을 고정하기 위해 사용하는 타입변수
		ArrayList<Music> list3 = new ArrayList<Music>(10);
		System.out.println(list3);
		
		// 1. add(E e) : 리스트의 끝 (리스트의 size)에 e를 저장하는 메서드 (리스트 끝!(잘알아두기) = 비어있는 공간 맨 앞부터 추가된다)
		list3.add(new Music("Drowning","WOODZ"));
		list3.add(new Music("너에게 닿기를","10CM"));
		list3.add(new Music("다시 만난 세계","소녀시대"));
		
		System.out.println(list3);
		
		// 2. add(int index, E e) : 인덱스 위치에 e값을 추가하는 메서드.
		list3.add(1, new Music("Whiplash","aespa"));
		// 배열의 용량(capacity)보다 더 많은 데이터를 저장하는 경우 기존배열보다 크기가 1 더 큰 배열을 생성한 후 깊은 복사를 한다.
		
		System.out.println(list3);
	
		// 3. set(int index, E e) : 리스트의 인덱스 위치(int index)의 값을 e로 "변경"시켜주는 메서드
		list3.set(0, new Music("너와의 모든 지금", "재쓰비"));
	
		System.out.println(list3);
		
		// 4. remove(int index) : 인덱스 위치(int index)에 저장된 데이터를 삭제시켜주는 메서드
		list3.remove(0);
		...
		// capacity: ArrayList의 내부 배열 크기, 기본적으로 데이터가 꽉 차지 않으면 자동으로 늘어나지 않는다. 값이 삭제되어도 용량은 그대로 유지.
		// size(): ArrayList에 저장된 실제 데이터의 개수. 데이터를 추가하거나 삭제할 때 데이터 개수만 변동.

		// 6. get(int index) : 인덱스 위치의 데이터를 반환해주는 메서드 (list3[0]<-이런식으로 값을 꺼내는 방식은 자바는 안씀
		Music m = list3.get(0);
		System.out.println();
		...
		// 8. indexOf(E e) : e요소가 컬렉션의 몇번 index에 있는지 확인하는 메서드.
		System.out.println(list4.indexOf(900)); // 존재하지 않는 값은 -1로 표시
		
		// equals메서드 재정의 후 필드값이 일치하므로 true
		System.out.println(list3.contains(new Music("너에게 닿기를","10CM")));
		
		System.out.println(new Music("너에게 닿기를","10CM").hashCode());
		System.out.println(new Music("너에게 닿기를","10CM").hashCode());
		...
		System.out.println("-----------------------------------");
		// Iterator 반복자
		// 컬렉션 내부 인자들을 Interator로 토큰단위로 관리.
		...
		// 9. subList(int index1, int index2) : 리스트의 index1부터 index2전까지의 데이터를 추출하여
		// 새로운 List를 반환해주는 메서드
		...
		Collections.sort(list5); // 기본정렬조건에 따라 정렬시켜주는 메서드. 오름차순 정렬 (가나다순)
