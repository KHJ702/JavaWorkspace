...
public interface Basic {
	...
	// (인터페이스 내부에서는) 기본 메서드를 항상 추상메서드로 간주하기 때문에 public abstract 생략가능
	...
	// 정적메서드(static(객체 생성없이 사용)는 사용 가능.
...
// 추상 클래스 - > 상속을 받아서 기능을 확장 (이건 물 스킬만 있었는데 불 스킬도 추가하려고 할 때 쓰는 느낌)
// 인터페이스 - > 동일한 실행 기능을 보장 (스킬이라는 것을 쓰는 것은 동일 하나 불,물 이런식으로 다른 경우 쓰는 느낌)